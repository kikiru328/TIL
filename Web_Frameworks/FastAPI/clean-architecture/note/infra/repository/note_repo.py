...
class NoteRepository(INoteRepository):
    ...
    def save(self, user_id: str, note_vo: NoteVO):
        with SessionLocal() as db:
            try:
                tags: list[Tag] = []
                for tag in note_vo.tags:  # N+1 query problem occurs
    ...
    def update(self, user_id: str, note_vo: NoteVO) -> NoteVO:
        with SessionLocal() as db:
            try:
                # delete all tags, prohibit orphan
    ...
    def get_notes_by_tag_name(
        ...
        with SessionLocal() as db:
            ...
            query = db.query(Note).filter(  # .options(joinedload(Note.tags))
